...
const connectToBridge = async () => {
  ...
  const bridge = await hueApi
    .createLocal(config.bridgeIp)
    .connect(config.bridgeUsername);
  ...
  const processState = _.throttle(() => {
    ...
    config.lights.forEach(async (light, index) => {
      ...
      if (
        _.has(currentState, light.dmxAddress) ||
        _.has(currentState, light.dmxAddress + 5)
      ) {
        ...
        // Turn the light off
        offLightState.off().transition(transitionTimeFromValue);
  ...
  sACN.on("packet", (packet) => {
    if (!_.isEqual(currentState, packet.payload)) {
      ...
      console.log("new state received", currentState);
      processState();
    }
  });
};
